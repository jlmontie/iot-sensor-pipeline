...
if CLOUD_MODE:
    title += " (Cloud)"
    st.markdown(
        "Real-time monitoring of agricultural IoT sensors powered by Google Cloud"
    )
else:
...
def fetch_data(query):
    ...
    try:
        ...
        if CLOUD_MODE and "db-dtypes" in str(e):
            ...
            st.error(
                "BigQuery table not found. Make sure data generator is running and has sent data."
            )
