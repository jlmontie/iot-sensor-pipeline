...
def parse_args():
    ...
    parser.add_argument('--project-id', 
                       help='GCP Project ID (required for cloud mode)')
...
if CLOUD_MODE:
    ...
    if not PROJECT_ID:
        print("Error: --project-id required for cloud mode (or set GCP_PROJECT_ID env var)")
        sys.exit(1)
...
class MessageProducer:
    ...
    def send_message(self, payload):
        ...
        if CLOUD_MODE:
            ...
            self.producer.produce(KAFKA_TOPIC, value=message_data, callback=self.delivery_report)
        ...
        while True:
            ...
            for sensor_id in sensors:
                ...
                if CLOUD_MODE:
                    ...
                    print(f"Sent {message_count} messages (Est. cost: ${estimated_cost:.4f})")
                else:
                    if message_count % (len(sensors) * 10) == 0:
                        print(f"Sent {message_count} messages ({message_count // len(sensors)} cycles)")
