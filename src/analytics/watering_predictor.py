...
class SmartWateringPredictor:
    ...
    def analyze(self, df):
        ...
        return self._compile_results(watering_event, prediction, health, recommendations, df_prepared)
    ...
    def _predict_next_watering_linear(self, df):
        ...
        if len(recent_data) >= 2:
            ...
            decay_rate = max(0.001, moisture_change / max(time_span_hours, 1))  # Avoid division by zero
            
            # Calculate hours until watering threshold
            hours_to_threshold = (current_moisture - self.watering_threshold) / decay_rate
            hours_to_threshold = max(1, min(336, hours_to_threshold))  # Clamp between 1 hour and 14 days
    ...
    def _fit_exponential_decay_model(self, df):
        ...
        if len(recent_data) < 10:
            return {'fitted': False, 'message': 'Insufficient data for exponential modeling'}
        ...
        try:
            ...
            return {
                ...
                'quality_metrics': {'r_squared': r_squared, 'confidence': self.confidence_score},
    ...
    def _create_features(self, moisture_history, time_features, environmental_data=None):
