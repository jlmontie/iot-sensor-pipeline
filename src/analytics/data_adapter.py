...
class IoTDataAdapter:
    ...
    def convert_sensor_data(self, sensor_data: pd.DataFrame, sensor_id: str) -> pd.DataFrame:
        """
        ...
        logger.info(f"Date range: {forecast_data['created_at'].min()} to {forecast_data['created_at'].max()}")
        logger.info(f"Moisture range: {forecast_data['moisture'].min():.3f} to {forecast_data['moisture'].max():.3f}")
